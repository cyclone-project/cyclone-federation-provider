<?php

$metadata[getenv('KEYCLOAK_URL') . '/realms/master'] = array (
    'AssertionConsumerService' => getenv('KEYCLOAK_URL') . '/realms/master/broker/DemoIDP/endpoint',
    'SingleLogoutService' =>  getenv('KEYCLOAK_URL') . '/realms/master/broker/DemoIDP/endpoint',
    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:persistent',
    'certData' => '',
    'signature.algorithm' => 'http://www.w3.org/2001/04/xmldsig-more#rsa-sha256',
    'validate.authnrequest' => false,
    'validate.logout' => false,
    'redirect.sign' => true,
    'redirect.validate' => true,
);